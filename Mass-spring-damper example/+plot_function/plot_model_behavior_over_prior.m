...
Hk = envir_data.H_k;
Ht = envir_data.H_t;
x_true = @(k,t) exp(-0.5*b*t).*(2*y0(2)/sqrt(4*k-b^2)*sin(0.5*sqrt(4*k-b^2)*t)+c2*cos(0.5*sqrt(4*k-b^2)*t));
...
set(gca,'Position',[lp(1) bp ww hh],'FontSize',12,'LineWidth',1.5,...
   'YLim',[-1.7,2.5],'YTick',-2:2,'XTick',0:3);
...
set(gca,'Position',[lp(2) bp ww hh],'FontSize',12,'LineWidth',1.5,'YTickLabel',[],...
   'YLim',[-1.7,2.5],'YTick',-2:2,'YTickLabel',[],'XTick',0:3);
text(0.4860,-0.0963,'\itt','Units','normalized','FontSize',15);
box on
